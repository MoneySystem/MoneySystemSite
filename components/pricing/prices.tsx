    ...
    (item: {
      ...
      return (
        <div className={`price ${item.isMostPopular ? 'most-popular' : ''}`}>
          ...
          <Link
            href={
              'https://api.whatsapp.com/send/?phone=5548988745520&text=Ol%C3%A1%21+Tenho+interesse+em+fazer+o+teste+gr%C3%A1tis'
            }
            className={'w-full'}
          >
  ...
  return (
    <div className={'prices flex flex-col gap-5'}>
      ...
      <TabPanel value={value} index={'yearly'}>
        <div className='flex w-full flex-wrap gap-6'>
          <div className='flex w-full gap-6 max-lg:flex-wrap'>
            {yearlyPrice.slice(0, 3).map((price, index) => renderPrice(price))}
          </div>
          <div className='flex w-full gap-6 max-lg:flex-wrap'>
            {yearlyPrice.slice(3).map((price, index) => renderPrice(price))}
          </div>
        </div>
      </TabPanel>
      <TabPanel value={value} index={'monthly'}>
        <div className='flex w-full flex-wrap gap-6'>
          <div className='flex w-full gap-6 max-lg:flex-wrap'>
            {monthlyPrice.slice(0, 3).map((price, index) => renderPrice(price))}
          </div>
          <div className='flex w-full gap-6 max-lg:flex-wrap'>
            {monthlyPrice.slice(3).map((price, index) => renderPrice(price))}
          </div>
        </div>
      </TabPanel>
    </div>
